...
def isStakeSubmissionScript(scriptVersion, script):
    ...
    return (
        extractStakePubKeyHash(script, stakeOpcode) != None
        or extractStakeScriptHash(script, stakeOpcode) != None
    )
...
def checkSStx(tx):
    ...
    for outTxIndex in range(1, len(tx.txOut)):
        ...
        if len(rawScript) < SStxPKHMinOutSize or len(rawScript) > SStxPKHMaxOutSize:
            raise Exception(
                "SStx output at output index %d was a NullData (OP_RETURN) push of the wrong size",
                outTxIndex,
            )
