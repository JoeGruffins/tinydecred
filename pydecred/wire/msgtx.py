...
class MsgTx:
    ...
    def decodePrefix(self, b, pver):  # r io.Reader, pver uint32) (uint64, error) {
        ...
        if count > maxTxOutPerMessage:
            raise Exception(
                "MsgTx.decodePrefixtoo many output transactions to fit into  max message size [count %d, max %d]"
    ...
    def decodeWitness(
        ...
        if not isFull:
            ...
            if count > maxTxInPerMessage:
                raise Exception(
                    "MsgTx.decodeWitness: too many input transactions to fit into max message size [count %d, max %d]"
                    % (count, maxTxInPerMessage)
                )
            ...
            if count != len(self.txIn):
                raise Exception(
                    "MsgTx.decodeWitness: non equal witness and prefix txin quantities (witness %v, prefix %v)"
                    % (count, len(self.txIn))
                )
        ...
        txIn=[
            TxIn(
                previousOutPoint=OutPoint(
                    txHash=None, idx=0xFFFFFFFF, tree=TxTreeRegular,
                ),
                sequence=0xFFFFFFFF,
            ),
        ],
...
def multiTx():
    ...
    return MsgTx(
        ...
        txIn=[
            TxIn(
                previousOutPoint=OutPoint(txHash=None, idx=0xFFFFFFFF, tree=0,),
                sequence=0xFFFFFFFF,
                valueIn=0x1212121212121212,
                blockHeight=0x15151515,
                blockIndex=0x34343434,
...
def multiTxEncoded():
    ...
    return ByteArray(
        [
            ...
            0x00,
            0x00,
            0x00,  # Transaction amount [47]
            0xAB,
            0xAB,  # Script version [55]
            ...
            0x00,
            0x00,
            0x00,  # Transaction amount [123]
            0xBC,
            0xBC,  # Script version [134]
